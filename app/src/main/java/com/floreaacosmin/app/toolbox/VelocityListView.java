...
// Modified by Cosmin Florea - October 2014 
...
public class VelocityListView extends AutoScrollListView {
    ...
    private final OnScrollListener mOnScrollListener = new OnScrollListener() {
        ...
        public void onScroll(AbsListView view, int firstVisiblePosition, int visibleItemCount, int totalItemCount) {
            ...
            if (mTime != INVALID_TIME) {
                ...
                if (now - mTime > MINIMUM_TIME_DELTA) {
                    ...
                    //@formatter:on
                        ...
                        /* We're in a case were the item we were previously referencing has moved 
                         * out of the visible window. Let's compute an approximative distance. */
                        int heightSum = 0;
                        for (int i = 0; i < visibleItemCount; i++) {
                            heightSum += getChildAt(i).getHeight();
                        }

                        /* If one of the elements used in the calculation is zero than set the 
                         * distance accordingly to zero as this is the logic outcome, this way 
                         * it is also avoided the dividing by zero case. */ 
                        if (heightSum == 0 || visibleItemCount == 0 || 
                        	(mFirstVisiblePosition - firstVisiblePosition) == 0) {
                        	distance = 0;
                        } else {
            ...
            /* Make this calculation only when the number of ListView elements is greater than one. 
             * The ListView header is counted as one of the elements. */
            if (getCount() > 1) {
